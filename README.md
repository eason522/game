...
- board cells now use a tighter inset-board material pass with wood-grain variation, recessed tile faces, smaller board gaps, thicker jade/ink stones, and stronger rock facets so the 11x11 grid reads as one crafted board instead of separate UI buttons
...
- battle feedback smoke tests for action logs, cell flashes, result banners, polished board frame, refined legend copy, material-style cell shadows, tightened board gaps, inset material renderer, and pseudo-3D rock visual state
