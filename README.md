...
- main menu smoke tests for default project entry, save-aware continue state, demo entry buttons, saved-run progress, saved-run closeout overview, state-aware continue action labels, the live-run launch check, the baseline playtest line, the live checklist action, the Boss focus line, the evidence line, the acceptance gate line, the archive/recap line, and the closeout line
...
3. Press Play to start at `scenes/ui/MainMenu.tscn`. The menu can continue a saved Run with a state-aware action label, preview saved-run progress plus its editor next action and closeout packet, show a one-line live-run launch check, automatic baseline playtest summary, first live checklist action, Boss focus line, evidence line, acceptance gate line, archive/recap line, and closeout line, start a new Run, or open `scenes/game/BattleScene.tscn` for a single battle.
