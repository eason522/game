...
- main menu and the run map now show a Demo editor rerun pack that combines visual checks, the full acceptance sequence, current status, archive review, and saved signature
- main menu and the run map now show a Demo editor preflight pack that checks route/sidebar scrolling, generated board/token assets, the current rerun action, and archive signature state before a real editor rerun
- the run map now scrolls both the route-node column and the long right-side build/acceptance panel so smaller windows no longer crop the Boss node or Demo rerun evidence
- 11x11 board
...
- battle feedback smoke tests for action logs, cell flashes, result banners, generated board and token texture loading, visual acceptance strip states, polished board frame, refined legend copy, zero-gap continuous board material, no per-tile button shadows, and pseudo-3D rock visual state
- main menu smoke tests for default project entry, save-aware continue state, demo entry buttons, saved-run progress, saved-run closeout overview, state-aware continue action labels, the live-run launch check, the baseline playtest line, the live checklist action, the Boss focus line, the Boss snapshot line, the evidence line, the acceptance gate line, the acceptance note line, the archive/recap line, the closeout line, the Demo acceptance packet, the Demo archive review packet, the archive audit signature, the editor rerun pack, and the editor preflight pack
- run map feedback smoke tests for typed settlement labels, tone triggers, live playtest checklist/verdict/review/Boss validation display, Boss snapshot pressure assessment, rest-focus feel audit, editor acceptance gates, editor next-action/evidence/note/archive/recap/closeout-packet guidance, Demo acceptance packet states, Demo archive review/audit states, editor rerun/preflight pack states, route/sidebar scrolling, priority verdicts, Boss feel recording, and tuning candidate display
- demo acceptance flow smoke tests for the stable rehearsal sample, save restore, main-menu review state, run-map acceptance packet, Boss feel/archive preservation, archive review/audit display, archive signature preservation, editor rerun/preflight pack preservation, and archive next action
