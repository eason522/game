...
func _refresh_build_summary() -> void:
	...
	var playtest_closeout_lines := playtest_simulator.get_live_run_closeout_lines(run_state)
	var boss_followup_lines := playtest_simulator.get_boss_pressure_followup_lines(run_state)
	var rest_focus_audit_lines := playtest_simulator.get_rest_focus_feel_audit_lines(run_state)
	var boss_live_checklist_lines := playtest_simulator.get_boss_live_checklist_lines(run_state)
	var editor_acceptance_lines := playtest_simulator.get_editor_run_acceptance_lines(run_state)
	var editor_next_action_lines := playtest_simulator.get_editor_next_action_lines(run_state)
	var editor_evidence_lines := playtest_simulator.get_editor_evidence_checklist_lines(run_state)
	var editor_note_lines := playtest_simulator.get_editor_acceptance_note_lines(run_state)
	...
	var demo_editor_preflight_lines := playtest_simulator.get_demo_editor_preflight_lines(run_state)
	...
	build_summary_label.text = "构筑效果：%s\nRun 节奏：%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n调参建议：%s\n基准试玩：%s\n实测对照：%s\n样本矩阵：%s\n矩阵落点：%s\n试玩检查：%s\n调参候选：%s" % [
		...
		" / ".join(playtest_review_lines),
		" / ".join(playtest_closeout_lines),
		" / ".join(boss_followup_lines),
		" / ".join(rest_focus_audit_lines),
		" / ".join(boss_live_checklist_lines),
		" / ".join(editor_acceptance_lines),
		" / ".join(editor_next_action_lines),
		" / ".join(editor_evidence_lines),
		" / ".join(editor_note_lines),
		" / ".join(editor_archive_lines),
		" / ".join(editor_recap_lines),
		" / ".join(editor_closeout_packet_lines),
		" / ".join(tuning_lines),
	...
	build_summary_label.text += "\n%s" % " / ".join(demo_editor_preflight_lines)
